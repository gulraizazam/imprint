...
class CustomersController extends Controller
{
    ...
    public function processLogin(Request $request)
    {
        ...
        if (auth()->guard('customer')->attempt($customerInfo)) {
            ...
            if($customer->verified == 0){
                ...
                return redirect()->back()->with('loginError', Lang::get("website.Please Verify Your Email"));
            }
    ...
    public function signupProcess(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (isset($res['email']) && $res['email'] == "true") {
                return redirect('/login')->withInput($request->input())->with('error', Lang::get("website.Email already exist"));
            }elseif(isset( $res['phone']) && $res['phone'] == "true"){
                return redirect('/login')->withInput($request->input())->with('error', Lang::get("website.Email already exist"));
            } else {
                if (isset($res['insert']) && $res['insert'] == "true") {
                    if ($res['auth'] == "true") {
                        $result = $res['result'];
                        Session::forget('guest_checkout');
                        ...
                        Auth::guard('customer')->logout();
                        if(isset($res['email'])){
                            return redirect()->intended('/login')->with('success', 'User Registered Successfully! Please Verify Your Email.');
                        }else{
                            return redirect('/verifycode')->with('success', 'User Registered Successfully! Please Enter Verification Code.');
                        }
    ...
    public function CodeVerification()
    {
         ...
         $final_theme = $this->theme->theme();
          $result['commonContent'] = $this->index->commonContent();
        return view('auth.codeverification',compact('result','final_theme'));
    }
    public function VerifyPhoneCode(Request $request)
    {
        $getCode = $request->code;
        $Convert = implode('', $getCode);
        $verifyCode = DB::table('verify_users')->where('token',$Convert)->first();
        $getUser = User::where('id', $verifyCode->user_id)->first();
        $getUser->verified =1;
         $getUser->update();
         return redirect()->intended('/login')->with('success', 'Verified Successfully.');
    }
}
