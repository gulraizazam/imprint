  ...
  $option = '';
    ...
    foreach($categories as $parents){
     $parent_slug  = $parents->slug;
      ...
      $getCategoriesCount = DB::table('products_to_categories')->join('products','products.products_id','=','products_to_categories.products_id')->where('products_to_categories.categories_id',$parents->categories_id)->get();
      $totalproducts = count($getCategoriesCount);
      ...
      $option .= '
       <a class=" main-manu"'. $hasChild .'> <img class="img-fuild" src="'.asset($parents->path).'"> '.$parents->categories_name.'<span class="procount" style="margin-left:35px;">('.$totalproducts.')</span></a>';
